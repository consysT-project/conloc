...
import de.tuda.stg.conloc.invariants.solver.subset.model.AbstractMethodModel;
import de.tuda.stg.conloc.invariants.solver.subset.model.BaseClassModel;
import de.tuda.stg.conloc.invariants.solver.subset.model.FieldModel;
...
public class MethodPostconditionExpressionParser extends MethodExpressionParser {
	...
	protected Expr parseJmlMessageSend(JmlMessageSend jmlMessageSend, int depth) {
		...
		if (JDTUtils.methodMatchesSignature(jmlMessageSend.actualReceiverType, methodBinding, true, "de.tuda.stg.conloc.invariants.utils.InvariantUtils", "stateful", "java.lang.Object")) {
	...
	public BoolExpr parseJmlAssignableClause(JmlAssignableClause jmlAssignableClause) {
		...
		if (jmlAssignableClause == null || jmlAssignableClause.expr.equals(JmlKeywordExpression.NOTHING)) {
			...
			for (FieldModel field : getClassModel().getFields()) {
				...
				if (field.getTypeBinding().isArrayType()) {
					// in this case we have an assignment to a default array
